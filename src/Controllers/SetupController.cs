...
namespace Keepix.SmartNodePlugin.Controllers
{
    internal class SetupController
    {
        ...
        [KeepixPluginFn("resync-eth1")]
        public static async Task<bool> OnResyncEth1()
        {
            var isDockerRunning = SetupService.isDockerRunning();
            if (!isDockerRunning) {
                Console.WriteLine("Docker is not live on your device, please start it");
            ...
            try
            {
                ...
                string error = string.Empty;
                if (isRunning) {
                    error = SetupService.ReSyncEth1Node();
                    if (!string.IsNullOrEmpty(error)) {
                        Console.WriteLine("An error occured while trying to resync-eth1 your node, check manually or contact the Keepix team " + error);
                    }
                }
            }
            catch (Exception) 
